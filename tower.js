...
function makeMove() {
  // Get the user's input from the "numDisk" input field
  const userInput = document.getElementById("numDisk").value;
  
  // Ensure the input is a valid integer
  const nDisks = parseInt(userInput);
  
  // Check if the input is a number
  if (isNaN(nDisks) || nDisks <= 0) {
    alert("Please enter a valid number of disks");
    return;
  }

  if (counter == 0) {
    // Use the nDisks variable (the user's input) instead of the hardcoded value
